...
class Waveguide(pya.PCellDeclarationHelper):
    def __init__(self):
        ...
        self.param(
            "CML", self.TypeString, "Compact Model Library (CML)", default="GSiP"
        )
        self.param(
            "model", self.TypeString, "CML Model name", default="wg_strip_integral_1550"
        )
    ...
    def produce_impl(self):
        ...
        text = Text(
            "Lumerical_INTERCONNECT_library=Design kits/%s" % self.CML,
            t,
            0.1 * wg_width,
            -1,
        )
        ...
        text = Text(
            ...
            % (waveguide_length * dbu, self.width, pts_txt, self.radius),
            t,
            0.1 * wg_width,
            -1,
        )
        ...
        text = Text("Length=%.6f" % (waveguide_length * dbu), t, 0.5 * wg_width, -1)
        text.halign = halign
        shape = self.cell.shapes(LayerDevRecN).insert(text)
