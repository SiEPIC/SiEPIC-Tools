...
class Ring_Filter_DB(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        ...
        layout_ring(
            self.cell,
            LayerSiN,
            pya.DPoint((self.r + self.w / 2), (self.r + self.g + self.w)),
            self.r,
            self.w,
        )
        # ring centre:
        t = pya.Trans(
            pya.Trans.R0, (self.r + self.w / 2) / dbu, (self.r + self.g + self.w) / dbu
        )
        ...
        # Create the MH Layer
        poly = (
            pya.Path(arc(self.r / dbu, angle_min_MH, angle_max_MH), w_mh)
            .transformed(t)
            .simple_polygon()
        )
        ...
        shapes(LayerPinRecN).insert(
            pya.Text(
                "opt1", pya.Trans(pya.Trans.R0, x0 - (w_Si3 / 2 + taper_length), 0)
            )
        ).text_size = 0.5 / dbu
        ...
        shapes(LayerPinRecN).insert(
            pya.Text(
                "opt2", pya.Trans(pya.Trans.R0, x0 + (w_Si3 / 2 + taper_length), 0)
            )
        ).text_size = 0.5 / dbu
        ...
        shapes(LayerPinRecN).insert(
            pya.Path(
                [
                    pya.Point(
                        x0 - (w_Si3 / 2 + taper_length) + pin_length / 2, y_offset
                    ),
                    pya.Point(
                        x0 - (w_Si3 / 2 + taper_length) - pin_length / 2, y_offset
                    ),
        ...
        shapes(LayerPinRecN).insert(
            pya.Text(
                "opt3",
                pya.Trans(pya.Trans.R0, x0 - (w_Si3 / 2 + taper_length), y_offset),
            )
        ).text_size = 0.5 / dbu
        ...
        shapes(LayerDevRecN).insert(
            pya.Text("Component=Ring_Filter_DB", pya.Trans(pya.Trans.R0, 0, w * 2))
        ).text_size = 0.3 / dbu
        shapes(LayerDevRecN).insert(
            pya.Text(
