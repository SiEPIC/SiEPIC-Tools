    ...
    * works for instances that are sub-circuits with many components, but one unique pin name
    ...
    if len(ipinA) == 1:
        ...
        if componentA == []:
            ...
            print(
                "parent_cell A: %s, cell A: %s"
                % (instanceA.parent_cell, instanceA.cell)
            )
            print(
    ...
    if len(ipinB) == 1:
        ...
        if componentB == []:
            ...
            print(
                "parent_cell B: %s, cell B: %s"
                % (instanceB.parent_cell, instanceB.cell)
            )
            print(
    ...
    if not (waveguide):
        ...
        if waveguide_type:
            ...
            if type(waveguide1) == type([]) and len(waveguide1) > 0:
                ...
                raise Exception(
                    "error: waveguide type (%s) not found in PDK. Waveguides available: %s"
                    % (waveguide_type, [w["name"] for w in waveguides])
                )
...
def connect_cell(
    ...
    componentA = instanceA.parent_cell.find_components(
        cell_selected=instanceA.cell, inst=instanceA, verbose=verbose
    )
    ...
    if componentA == []:
        if verbose:
            print(
                "*** WARNING: componentA not found, looking higher in the hierarchy which may not work correctly: instanceA.parent_cell.find_components(inst=instanceA)"
            )
        componentA = instanceA.parent_cell.find_components(inst=instanceA)
        if componentA == []:
            if _globals.Python_Env == "KLayout_GUI":
    ...
    def error_pinA(pinA, componentA):
        ...
        error_message = (
            ...
            % (
                ...
                str(getframeinfo(stack()[2][0])).replace(",", "\n"),
            )
        )
        """
    if _globals.Python_Env == "KLayout_GUI":
...
def user_select_opt_in(
    cell=None, verbose=None, option_all=True, opt_in_selection_text=[]
):
    """
...
def replace_cell(
    ...
    if not Exact:
        ...
        cells_x += [
            ...
            if cell.name.find(cell_x_name + RequiredCharacter) == 0
        ]
...
def instantiate_all_library_cells(
    topcell,
    terminator_cells=None,
    terminator_libraries=None,
    terminator_waveguide_types=None,
    progress_bar=True,
):
    """
    ...
    for lib in pya.Library().library_ids():
        ...
        for n in li.layout().pcell_names():
            ...
            if pcell:
                ...
                t = pya.Trans(
                    pya.Trans.R0, x - subcell.bbox().left, y - subcell.bbox().bottom
                )
