            ...
            while not (iter2.at_end()):
                ...
                if pin_name and pin_path.num_points() == 2:
                    ...
                    if it.shape().cell.name == iter2.shape().cell.name:
                        ...
                        if verbose:
                            print(
                                " - found pin: %s in cell %s, in %s, text %s"
                                % (
                                    pin_name,
                                    subcell.name,
                                    it.shape().cell.name,
                                    iter2.shape().cell.name,
                                )
                            )
                iter2.next()
        ...
        if p:
            if len(p) > 1:
                raise Exception(
                    ...
                    % (pin_name, self.basic_name())
                )
            return p[0]
        else:
            raise Exception(
                'Pin with name "%s" not found in cell "%s". Available pins: %s'
                % (pin_name, self.basic_name(), [p.pin_name for p in pins])
            )
    else:
...
def find_components(
    self, cell_selected=None, inst=None, verbose=False, raiseException=True
):
    """
    ...
    if (
        cell_selected != None
        and type(cell_selected) != type([])
        and type(cell_selected) != tuple
    ):
    ...
    if verbose:
        ...
        if cell_selected[0]:
            print(
                "  - cell_selected=%s"
                % (cell_selected[0].name if cell_selected[0] else None)
            )
        if inst:
    ...
    while not (iter1.at_end()):
        ...
        if cell_selected and subcell not in cell_selected:
            ...
            if verbose:
                print(
                    " - cell_selected and not subcell (%s) in cell_selected (%s). "
                    % (subcell.name, cell_selected[0].name)
                )
            iter1.next()
            continue
        ...
        if iter1.shape().is_path():
            polygon = (
                iter1.shape().path.polygon().transformed(iter1.itrans())
    ...
    if components == []:
        if raiseException:
            raise Exception(
                "SiEPIC.extend.find_components: No component found for cell_selected=%s"
                % (cell_selected[0].name if cell_selected else None)
            )
        else:
            return None
    return components
...
def show(self):
    ...
    if Python_Env == "Script":
        ...
        klive.show(
            file_out, technology=self.layout().technology().name, keep_position=True
        )
...
def plot(self, width=800, show_labels=True, show_ruler=True, retina=True):
    ...
    pixel_buffer = layout_view.get_pixels(
        width, cell.bbox().height() / cell.bbox().width() * width
    )
...
def find_pin(self, pin_name, verbose=False):
    ...
    if pins:
        ...
        if p:
            ...
            raise Exception(
                'Pin with name "%s" not found in cell "%s". Available pins: %s'
                % (pin_name, self.cell.basic_name(), [p.pin_name for p in pins])
            )
    else:
