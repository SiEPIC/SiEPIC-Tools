...
def layout_check(
    cell=None, verbose=False, GUI=False, timing=False, file_rdb=None, verify_DFT=True
):
    ...
    if DFT:
        ...
        if "minimum-gc-spacing" in DFT["design-for-test"]["grating-couplers"].keys():
            rdb_cat_id_GC_min_spacing = rdb.create_category(
                rdb_cat_id, "Grating coupler: minimum spacing"
            )
            rdb_cat_id_GC_min_spacing.description = (
                "The grating coupler spacing (pitch) must be at least %s microns."
                % float(
                    DFT["design-for-test"]["grating-couplers"]["minimum-gc-spacing"]
                )
            )
        ...
        for e in extra_shapes:
            if e[0].dpolygon:
                if verbose:
                    ...
                    print(
                        "    - %s, %s" % (e[1].to_trans().to_itrans(dbu), e[0].dpolygon)
                    )
                rdb_item = rdb.create_item(
                    rdb_cell.rdb_id(), rdb_cat_id_comp_shapesoutside.rdb_id()
                )
                rdb_item.add_value(
                    pya.RdbItemValue(e[0].polygon.transformed(e[1]).to_dtype(dbu))
                )
    ...
    for i in range(0, len(components)):
        ...
        for pin in c.pins:
            if pin.type == _globals.PIN_TYPES.OPTICAL:
                ...
                if not polygon_and:
                    ...
                    rdb_item.add_value(
                        pya.RdbItemValue(pya.Polygon(test_box).to_dtype(dbu))
                    )
                    rdb_item.add_value(
                        pya.RdbItemValue(
        ...
        if DFT:
            ...
            if c.basic_name:
                ...
                if (
                    "minimum-gc-spacing"
                    in DFT["design-for-test"]["grating-couplers"].keys()
                ):
                    test = [
                        ci.startswith(k)
                        for k in DFT["design-for-test"]["grating-couplers"][
                            "gc-orientation"
                        ].keys()
                    ]
                    if any(test):
                        ...
                        for i2 in range(i + 1, len(components)):
                            ...
                            if any(test):
                                ...
                                if dist < min_gc_spacing:
                                    rdb_item = rdb.create_item(
                                        rdb_cell.rdb_id(),
                                        rdb_cat_id_GC_min_spacing.rdb_id(),
                                    )
                                    rdb_item.add_value(
                                        pya.RdbItemValue(
                                            "Grating couplers should be at least %s microns apart (center-to-center pitch)"
                                            % (min_gc_spacing)
                                        )
                                    )
                                    rdb_item.add_value(
                                        pya.RdbItemValue(c.polygon.to_dtype(dbu))
                                    )
                                    rdb_item.add_value(
                                        pya.RdbItemValue(c2.polygon.to_dtype(dbu))
                                    )
                ...
                if components_sorted:
                    ...
                    if (
                        ...
                        * 1000
                        ...
                        rdb_item.add_value(
                            pya.RdbItemValue(components_sorted[0].polygon.to_dtype(dbu))
                        )
    ...
    if rdb.num_items() > 0:
        ...
        for e in rdb.each_item():
            print(
                "Error: %s: %s"
                % (
                    rdb.category_by_id(e.category_id()).name(),
                    rdb.category_by_id(e.category_id()).description,
                )
            )
    else:
        if Python_Env == "KLayout_GUI":
