...
def create_cell2(ly, cell_name, library_name, load_check=True):
    ...
    if not pcell:
        if library_name not in pya.Library().library_names():
            raise Exception(
                "Error: library (%s) not available. Libraries for technology (%s) are: %s."
                % (library_name, ly.technology().name, pya.Library().library_names())
            )
        ly_library = (
            pya.Library().library_by_name(library_name, ly.technology().name).layout()
        )
        library_cells = [ly_library.cell(a).name for a in ly_library.each_top_cell()]
        if cell_name not in library_cells:
            raise Exception(
                "Error: cell (%s) not available in library (%s) for technology (%s). Cells are: %s."
                % (cell_name, library_name, ly.technology().name, library_cells)
            )

        raise Exception(
            "Error: loading cell (%s) from library (%s)" % (cell_name, library_name)
        )
...
def selected_opt_in_text():
    ...
    if Python_Env == "Script":
        raise Exception(
            "This function can only be executed in KLayout Application GUI mode."
        )
...
def arc_bezier(radius, start, stop, bezier, DevRec=None, dbu=0.001):
    ...
    for i in range(1, N - 1):
        ...
        pts.append(
            pya.Point(-L, 0)
            + pya.Point(
                t**3 * xA + t**2 * xB + t * xC + xD, t**3 * yA + t**2 * yB + t * yC + yD
            )
        )
...
def translate_from_normal(pts, trans):
    ...
    if type(pts[0]) == pya.Point:
        ...
        raise Exception(
            "SiEPIC.utils.translate_from_normal expects pts=[pya.Point,...] or [pya.DPoint,...]"
        )
    if len(pts) < 2:
        return pts
...
def waveguide_length(cell):
    ...
    if type(cell) == pya.Cell:
        return (
            float(cell.find_components()[0].params.split("wg_length=")[1].split(" ")[0])
            * 1e6
        )
    else:
        raise Exception(
            "SiEPIC.utils.waveguide_length: input needs to be a Cell or Instance."
        )
