...
def layout_waveguide2(
    ...
    for lr in range(0, len(layers)):
        ...
        if len(wg_pts) < 2:
            print(
                " - warning: SiEPIC.utils.layout.layout_waveguide2: less than 2 points."
            )
            return 0
        wg_polygon = Polygon(
...
def y_splitter_tree(
    ...
    cell_tree.transform(
        pya.Trans(-inst_in.pinPoint("opt1").x, -inst_in.pinPoint("opt1").y)
    )
...
def strip2rib(
    cell, trans, w_slab, w_rib, w_slab_tip, w_strip, length, LayerRib, LayerSlab
):
    """
    ...
    poly = DPolygon(
        [
            DPoint(length, -w_slab / 2),
            DPoint(length, w_slab / 2),
            DPoint(0, w_slab_tip / 2),
            DPoint(0, -w_slab_tip / 2),
        ]
    )
